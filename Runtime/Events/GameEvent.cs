...
namespace Core.Infrastructure.Events
{
    ...
    {
        private readonly List<Action<T>> _listeners = new(64);
    ...
    {
        private readonly List<Action<T1, T2>> _listeners = new(64);
        ...
        {
            foreach (var listener in _listeners) listener?.Invoke(eventData1, eventData2);
        }
    }

    public abstract class GameEvent<T1, T2, T3> : GameEvent
    {
        private readonly List<Action<T1, T2, T3>> _listeners = new(64);
        ...
        {
            foreach (var listener in _listeners) listener?.Invoke(eventData1, eventData2, eventData3);
        }
    }
}